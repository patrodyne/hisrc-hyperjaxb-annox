...
Next, the [Main][25] and [PurchaseOrderTest][33] classes each create a `JAXBContext` with an `AnnoxAnnotationReader` to read [`ann.xml`][4] resource files and rebind the annotations in memory. Thus, the generated **JAXB** classes become *Plain Old Java Objects (POJOs)* while the context is provided in memory.
